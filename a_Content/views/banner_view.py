...
from a_Content.forms.banner_forms import FactoryATBannerForm
...
@PermissionRequired('create', 'ATBanner')
def create_banner(request, url):
    ...
    CreateForm = FactoryATBannerForm.get_class('create')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            data['tipo'] = ContentType.ATBANNER
...
@PermissionRequired('update', 'ATBanner')
def update_banner(request, url):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            data = form.cleaned_data
            data['id'] = content.id
            result, message, link = ServiceDispatcher.dispatch(
                type_,
                action,
                data=data
            )
            ...
            args = [url]
            if result == 'success':
                service_log.log_update(link, content) # Registra no serviço de log
                if parent:
        ...
        form = CreateForm(initial={
            ...
            'target' : banner_data.get('target'),
            'imagem_destaque' : banner_data.get('imagem_destaque')
        })
...
@PermissionRequired('delete', 'ATBanner')
def delete_banner(request, url):
...
@PermissionRequired('workflow', 'ATBanner')
def workflow_banner(request, url):
